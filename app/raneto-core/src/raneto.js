...
class Raneto {
  ...
  doSearch(query) {
    ...
    results.forEach(result => {
      ...
      const trailingElipsis = end === page.text.length ? '' : '...';
      const substring = page.text.substring(beginning, end).trim();
      if (!substring.length) {
        return;
      }

      const baseExcerpt = leadingElipsis + substring + trailingElipsis;
